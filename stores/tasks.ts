...
export interface Task {
  ...
  type: 'Feature' | 'Refactor' | 'Design' | 'Performance' | 'Documentation' | 'Accessibility' | 'Testing' | 'Security'
...
export const useTasksStore = defineStore('tasks', () => {
  ...
  const fetchTasks = async () => {
    loading.value = true
    error.value = null
    const { data, error: fetchError } = await client.from('tasks').select('*').order('created_at', { ascending: false })
  ...
  const updateTask = async (id: string, updates: Partial<Task>) => {
    loading.value = true
    error.value = null
    const { error: updateError } = await client.from('tasks').update(updates).eq('id', id)
  ...
  const deleteTask = async (id: string) => {
    loading.value = true
    error.value = null
    const { error: deleteError } = await client.from('tasks').delete().eq('id', id)
